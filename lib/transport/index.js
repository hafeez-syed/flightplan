...
function Transport(context) {
  ...
  commands.forEach(function(command) {
    self[command] = function(extension, options) {
      if(typeof extension === 'object') {
        options = extension;
        extension = null;
      }
      var _command = format('%s%s', this._execWith, command);
      _command = extension ? format('%s %s', _command, extension) : _command;
      return self._exec(_command, options);
    };
  });
}
 ...
 * @return input
 */
Transport.prototype.prompt = (function() {
  var queue = [];
  var queueCallbacks = [];
  var queueIsRunning = false;

  var next = function(doneCallback) {
    if(doneCallback) {
      queueCallbacks.push(doneCallback);
  ...
  return function(message, options) {
    ...
    return this.waitFor(function(done) {
      queue.push(function() {
        ...
        prompt.get({
          name: 'input',
          description: ' [prompt] ' + chalk.white(message),
          ...
          next(function() { done(result ? result.input : null); });
        });
      });
      next();
    });
